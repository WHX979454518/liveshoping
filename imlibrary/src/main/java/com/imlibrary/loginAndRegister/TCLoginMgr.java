...
public class TCLoginMgr {
    ...
    public void init(Context context) {
        ...
        mTLSLoginHelper = TLSLoginHelper.getInstance().init(context,
                TLSConstant.SDK_APPID, TLSConstant.ACCOUNT_TYPE,  TLSConfiguration.APP_VERSION);
...
//                imLogin(com.hotniao.live.HnApplication.getmUserBean().getTim().getAccount(), com.hotniao.live.HnApplication.getmUserBean().getTim().getSign(), tlsUserInfo.identifier, getUserSig(tlsUserInfo.identifier));
            }
...
//            imLogin(com.hotniao.live.HnApplication.getmUserBean().getTim().getAccount(), com.hotniao.live.HnApplication.getmUserBean().getTim().getSign(), getLastUserInfo().identifier, getUserSig(getLastUserInfo().identifier));
        }
        return true;
    }
    ...
    public void imLogin(String identify, String userSig,String appid,String type) {
        if(TextUtils.isEmpty(identify)||TextUtils.isEmpty(userSig)||TextUtils.isEmpty(appid)||TextUtils.isEmpty(type))return;
        TCConstants.setAppid(appid);
        ...
        //发起登录请求
        TIMManager.getInstance().login(Integer.parseInt(appid), user, userSig, new TIMCallBack() {
            @Override
