...
public class NiceVideoPlayer extends FrameLayout
    ...
    private MediaPlayer.OnVideoSizeChangedListener mOnVideoSizeChangedListener
        ...
        public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
            ...
            if(width<800){
                ...
                updateTextureViewSize(mVideoMode,CENTER_MODE,2);
                if (listener!=null){
    ...
    public void enterFullScreen() {
        ...
        mVideoWidth = mMediaPlayer.getVideoWidth();
        if(mVideoWidth<800){
            updateTextureViewSize(mVideoMode,CENTER_MODE,1);
            if (listener!=null){
                listener.onVideoSizeChanged(mVideoWidth,mVideoHeight);
            }
        }
    ...
    public boolean exitFullScreen() {
        if (mPlayerState == PLAYER_FULL_SCREEN) {
            ...
            mVideoWidth = mMediaPlayer.getVideoWidth();
            if(mVideoWidth<800){
                updateTextureViewSize(mVideoMode,CENTER_MODE,4);
                if (listener!=null){
                    listener.onVideoSizeChanged(mVideoWidth,mVideoHeight);
                }
            }
...
//                mVideoHeight = mMediaPlayer.getVideoHeight();
//                mVideoWidth = mMediaPlayer.getVideoWidth();
//                updateTextureViewSize(mVideoMode,CENTER_MODE,3);
//                if (listener!=null){
//                    listener.onVideoSizeChanged(mVideoWidth,mVideoHeight);
//                }
//            }
     ...
     * value is 0.
     */
    public void updateTextureViewSize(int mode,int type,int typee){
        //typee用来判断进入全屏和退出全屏的时候用的
        if(type ==CENTER_CROP_MODE){
            updateTextureViewSizeCenterCrop();
        } else if(mode==CENTER_MODE&&typee==4){
            updateTextureViewSizeCenter(4);
        }
        else if (mode==CENTER_MODE&&typee==1){
            updateTextureViewSizeCenter((float) 1.8);
        } else if (mode==CENTER_MODE&&typee==3){
            updateTextureViewSizeCenter(2);
    ...
    private void updateTextureViewSizeCenterCrop(){
        ...
        //过的部分超出了View的范围,所以是不会显示的,相当于裁剪了.
        matrix.postScale(maxScale, maxScale, getWidth() / 2, getHeight() / 2);//后两个参数坐标是以整个View的坐标系以参考的
        mTextureView.setTransform(matrix);
        postInvalidate();
    }
    //重新计算video的显示位置，让其全部显示并据中(成的视频不做处理，一般举重，抖音小视频处理)
    private void updateTextureViewSizeCenter(float type){
        float sx = (float) getWidth() / (float) mVideoWidth;
        float sy = (float) getHeight() / (float) mVideoHeight;
        Matrix matrix = new Matrix();
        //第1步:把视频区移动到View区,使两者中心点重合.
        if(type==4){
            matrix.preTranslate((float) ((getWidth() - mVideoWidth) / 2.5-mVideoWidth), (getHeight() - mVideoHeight)+200);
        }else if(type<2){
            matrix.preTranslate((getWidth() - mVideoWidth) / type+mVideoWidth, (getHeight() - mVideoHeight) / 2);
        }
        else {
            matrix.preTranslate((getWidth() - mVideoWidth) / type, (getHeight() - mVideoHeight) / 2);
        }

        //第2步:因为默认视频是fitXY的形式显示的,所以首先要缩放还原回来.
        if(type==4){
            matrix.preScale((float) (mVideoWidth / (float) getWidth()*2), mVideoHeight / (float) getHeight());
        }else {
            matrix.preScale(mVideoWidth / (float) getWidth(), mVideoHeight / (float) getHeight());
        }
