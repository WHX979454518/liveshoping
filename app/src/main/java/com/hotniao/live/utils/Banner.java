...
import com.live.shoplib.utils.banner.MVideoView;
...
public class Banner extends RelativeLayout
{
    ...
    //图片默认时间间隔
    private int imgDelyed = 200000;
    //每个位置默认时间间隔，因为有视频的原因
    private int delyedTime = 0;
    //默认显示位置
    ...
    NiceVideoPlayer videoView = null;
    View view1 = null;
...
//        options.centerCrop();
        ...
        if (dataList.size() > 1)
        {
            ...
            for (int i = 0; i < dataList.size() + 2; i++)
            {
                ...
                {
                    videoView = new NiceVideoPlayer(getContext());
                    videoView.setVideoMode(NiceVideoPlayer.CENTER_MODE);

                    NiceVideoPlayerController controller = new NiceVideoPlayerController("1",getContext());
                    controller.setTitle("");
    ...
    public void startBanner()
    {
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener()
        {
            ...
            public void onPageScrollStateChanged(int state)
            {
                ...
                //停止滑动时，重新自动倒计时
                view1 = views.get(pageIndex);
                if (view1 instanceof VideoView){
                    videoView = (NiceVideoPlayer) view1;
//                    videoView.setVideoMode(NiceVideoPlayer.CENTER_MODE);
                }
                if (state == 0 && views.size() > 1){
...
//                        videoView.setVideoMode(NiceVideoPlayer.CENTER_MODE);
    ...
    private class Time implements Runnable {
        ...
        public void getDelyedTime(NiceVideoPlayer videoView, Runnable runnable){
...
//            videoView.seekTo(0);
    ...
    public void destroy(){
        ...
        if(null!=videoView){
            if (videoView.isPlaying()
                    || videoView.isPaused()
                    || videoView.isBufferingPlaying()
                    || videoView.isBufferingPaused()) {
                videoView.start();
            }else {
                videoView.start();
            }
    ...
    public void onpause(){
        if(null!=videoView){
            videoView = (NiceVideoPlayer) view1;
//            videoView.setVideoMode(NiceVideoPlayer.CENTER_MODE);
            imageView = videoView.findViewById(R.id.back);
            if(null!=imageView){
