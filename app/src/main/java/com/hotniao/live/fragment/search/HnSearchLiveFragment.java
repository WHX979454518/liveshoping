...
public class HnSearchLiveFragment extends CommListFragment {
    ...
    public static HnSearchLiveFragment getInstance(String keyWord) {
        HnSearchLiveFragment f=new HnSearchLiveFragment();
        Bundle bundle=new Bundle();
        bundle.putString("kw",keyWord);
    ...
    protected CommRecyclerAdapter setAdapter() {
        ...
        setGridManager(true);
        mKeyWords=getArguments().getString("kw");

        mLoadingLayout.setStatus(HnLoadingLayout.Success);
        mAdapter=new CommRecyclerAdapter() {
            @Override
            protected void onBindView(BaseViewHolder holder, final int position) {
                ((FrescoImageView)holder.getView(R.id.mIvImg)).setImageURI(HnUrl.setImageUri(mData.get(position).getAnchor_live_img()));
                //直播模式  vip/计时/门票
                ...
                if(!"0".equals(mData.get(position).getAnchor_game_category_id())){
                    ( (FrescoImageView)holder.getView(R.id.mIvGameLogo)).setImageURI(HnUrl.setImageUri(mData.get(position).getAnchor_game_category_logo()));
                ...
                holder.getView(R.id.mIvImg).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        HnLiveSwitchDataUitl.joinRoom(mActivity,mData.get(position).getUser_id(),"");
                    }
                });
            }
