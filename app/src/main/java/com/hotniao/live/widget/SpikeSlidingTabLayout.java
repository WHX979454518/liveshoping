 ...
 */
public class SpikeSlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    private String TAG = "SpikeSlidingTabLayout";
    private Context mContext;
    private ViewPager mViewPager;
    private ArrayList<SpikeTitleModel.SpikeTitleItem> mTitles;
    ...
    public SpikeSlidingTabLayout(Context context) {
        this(context, null, 0);
    }

    public SpikeSlidingTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SpikeSlidingTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    public void setViewPager(ViewPager vp, SpikeTitleModel.SpikeTitleItem[] titles) {
        if (vp == null || vp.getAdapter() == null) {
            throw new IllegalStateException("ViewPager or ViewPager adapter can not be NULL !");
        }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.layout_spike_tab, null);
            CharSequence pageTitle = mTitles == null ? mViewPager.getAdapter().getPageTitle(i) : mTitles.get(i).getTitle();
            CharSequence pageSecondTitle = mTitles == null ? mViewPager.getAdapter().getPageTitle(i) : mTitles.get(i).getSecondTitle();
            addTab(i, pageTitle.toString(), pageSecondTitle.toString(), tabView);
        }

        updateTabStyles();
    }

//    public void addNewTab(String title, String secondTitle) {
//        View tabView = View.inflate(mContext, R.layout.layout_spike_tab, null);
//        if (mTitles != null) {
//            mTitles.add(new SpikeTabEntity(title, secondTitle));
//        }
//
//        CharSequence pageTitle = mTitles == null ? mViewPager.getAdapter().getPageTitle(mTabCount) : mTitles.get(mTabCount).getTitle();
//        CharSequence pageSecondTitle = mTitles == null ? mViewPager.getAdapter().getPageTitle(mTabCount) : mTitles.get(mTabCount).getSecondTitle();
//        addTab(mTabCount, pageTitle.toString(), pageSecondTitle.toString(), tabView);
//        this.mTabCount = mTitles == null ? mViewPager.getAdapter().getCount() : mTitles.size();
//
//        updateTabStyles();
//    }
     ...
     * 创建并添加tab
     */
    private void addTab(final int position, String title, String secondTitle, View tabView) {
        TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        TextView tv_tab_title_second = (TextView) tabView.findViewById(R.id.tv_tab_title_second);
        if (tv_tab_title != null) {
            if (title != null) tv_tab_title.setText(title);
        }
        if (tv_tab_title_second != null) {
            if (secondTitle != null) tv_tab_title_second.setText(secondTitle);
        }
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (tv_tab_title != null) {
                String info = i == mCurrentTab ? "spike_select" : "spike_unselect";
                HnLogUtils.d(TAG, "updateTabStyles当前：" + i + "颜色：" + info);

                v.setBackgroundColor(i == mCurrentTab ? getResources().getColor(R.color.main_color) : getResources().getColor(R.color.spike_unselect));
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                tabView.setBackgroundColor(isSelect ? getResources().getColor(R.color.main_color) : getResources().getColor(R.color.spike_unselect));
