...
class PinyinComparator<T extends IndexableEntity> implements Comparator<EntityWrapper<T>> {
    ...
    private String getWord(String indexName, int index) {
        ...
        if (PinyinUtil.matchingPolyphone(indexName)) {
            firstWord = PinyinUtil.getPinYin(PinyinUtil.getPolyphoneRealHanzi(indexName).substring(index, index + 1));
        } else {
            firstWord = PinyinUtil.getPinYin(indexName.substring(index, index + 1));
        }
        return firstWord;
    }
}
