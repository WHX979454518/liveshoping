...
public class HnAudinceRequestBiz extends HnLiveBaseRequestBiz {
     ...
     */
    public void requestToPay(String mAnchor_id, String type) {
        RequestParams param = new RequestParams();
        ...
        HnHttpUtils.postRequest(url, param, TAG, new HnResponseHandler<HnPayRoomPriceModel>(context, HnPayRoomPriceModel.class) {
            @Override
            public void hnSuccess(String response) {
                if (model.getC() == 0) {
                    if (listener != null) {
                        listener.requestSuccess("pay_room_price", response, model);
                    }
                } else {
                    if (listener != null) {
            ...
            HnHttpUtils.postRequest(HnLiveUrl.Join_To_Room, param, uid, new HnResponseHandler<HnLiveRoomInfoModel>(context, HnLiveRoomInfoModel.class) {
                ...
                public void hnSuccess(String response) {
                    if (model.getC() == 0) {
                        ...
                        if (listener != null) {
                            listener.requestFail("join_live_room", model.getC(), uid);
                        }
                    }
                }
                ...
                public void hnErr(int errCode, String msg) {
                    if (listener != null) {
                        listener.requestFail("join_live_room", errCode, uid);
                    }
                }
            });
        } catch (Exception e) {
