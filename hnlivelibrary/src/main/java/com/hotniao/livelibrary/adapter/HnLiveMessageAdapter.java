...
public class HnLiveMessageAdapter extends BaseAdapter {
    ...
    private static final int RICH_LV_TYPE = 15;  //用户升级通知
    ...
    private static final int System = 21;  //系统
    ...
    public int getItemViewType(int position) {
        ...
        } else if ((HnWebscoketConstants.Level_Up).equalsIgnoreCase(type)) {   //用户升级
            return RICH_LV_TYPE;
...
//        HnGagHolder gagholder = null;  //禁言
        ...
        HnRichLvHolder hnRichLvHolder = null;  //用户升级
        ...
        if (convertView == null) {
            switch (itemViewType) {
                ...
                case SHARE_SUC_TYPE: //分享成功
                    ...
                    break;
                case RICH_LV_TYPE:  //用户升级
                    convertView = View.inflate(context, R.layout.live_item_anchor_room_notify, null);
                    hnRichLvHolder = new HnRichLvHolder(convertView);
                    convertView.setTag(hnRichLvHolder);
                    break;
            ...
            switch (itemViewType) {
                ...
                case SHARE_SUC_TYPE:  //分享成功
                    ...
                    break;
                case RICH_LV_TYPE:  //用户升级
                    hnRichLvHolder = (HnRichLvHolder) convertView.getTag();
                    break;
                case ANCHOR_LV_TYPE:  //主播升级
                    ...
                    if (user.getData().getChat() != null && user.getData().getUser() != null) {
                        ...
                        if (!TextUtils.isEmpty(content)) {
                            ...
                            if (!TextUtils.isEmpty(userLv) && !TextUtils.isEmpty(nick)) {
                                msgholder.mLevelView.setVisibility(View.VISIBLE);
                                msgholder.mLevelView.setLevel(Integer.valueOf(fuser.getUser_level()));
//                                HnLiveLevelUtil.setAudienceLevBg(msgholder.mLevel, userLv, true);
                                ...
                                //设置字体前景色
                                sb.setSpan(new ForegroundColorSpan(context.getResources().getColor(R.color.comm_text_color_main)),
                                        0, nick.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); //设置前景色为洋红色
                    ...
                    if (user.getData().getChat() != null && user.getData().getUser() != null) {
                        ...
                        if (!TextUtils.isEmpty(content)) {
                            ...
                            if (!TextUtils.isEmpty(userLv) && !TextUtils.isEmpty(nick)) {
                                ...
                                SpannableStringBuilder sb = EmojiUtil.handlerEmojiText(nick, nick + content, context);
                                //设置字体前景色
                                sb.setSpan(new ForegroundColorSpan(context.getResources().getColor(R.color.main_color)),
                                        0, nick.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); //设置前景色为洋红色
            ...
            case SENDGIFT_TYPE:  //礼物
                ...
                if (gift != null && !TextUtils.isEmpty(gift.getLive_gift_name())) {
                    ...
                    sendGiftholder.mLevelView.setVisibility(View.VISIBLE);
                    sendGiftholder.mLevelView.setLevel(Integer.valueOf(user.getData().getUser().getUser_level()));
                    tipTemplate(SENDGIFT_TYPE, sendGiftholder, user,
                            context.getString(R.string.live_zeng_song_one_gitf) + gift.getLive_gift_nunmer() + context.getString(R.string.live_num) + giftName,
                            context.getResources().getColor(R.color.comm_text_color_main),
                            context.getResources().getColor(R.color.comm_live_gife_msg), false, false);
                }
                break;
            case NOTIFY_TYPE:  //公告
                inflateContent(notifyholder.mMsg, context.getString(R.string.live_sys_notice), user.getNotice(),
                        context.getResources().getColor(R.color.comm_live_notice_msg),
                        context.getResources().getColor(R.color.comm_live_notice_msg));
                break;
            case KLICK_TYPE:  //踢人
                if (user != null) {
                    ...
                    if (data != null) {
                        ...
                        if (user1 != null) {
                            if (!TextUtils.isEmpty(user1.getUser_id())) {
                                inflateContent(notifyholder.mMsg, user1.getUser_nickname(), context.getString(R.string.live_anchor_kick_room),
                                        context.getResources().getColor(R.color.comm_live_take_out),
                                        context.getResources().getColor(R.color.comm_live_take_out));
                            }
                        }
                    }
            ...
            case GAG_TYPE:  //禁言
                    ...
                    if (data != null) {
                        ...
                        if (fuser != null) {
                            if (!TextUtils.isEmpty(fuser.getUser_id())) {
                                inflateContent(notifyholder.mMsg, fuser.getUser_nickname(),
                                    context.getString(R.string.live_anchor_not_speak),
                                    context.getResources().getColor(R.color.comm_live_bind_chat),
                                    context.getResources().getColor(R.color.comm_live_bind_chat));
                            }
                        }
                    }
                ...
                break;
            case PRISE_TYPE:  //点赞
                priseholder.mLevelView.setVisibility(View.VISIBLE);
                priseholder.mLevelView.setLevel(Integer.valueOf(user.getData().getUser().getUser_level()));
                tipTemplate(PRISE_TYPE, priseholder, user, context.getString(R.string.live_liked_anchor),
                        context.getResources().getColor(R.color.comm_text_color_main),
                        context.getResources().getColor(R.color.comm_live_dianzan), false, false);
                break;
            case ADDADMIN_TYPE:  //添加管理员
...
//                tipTemplate(hnCancelAdminholder, user, context.getString(R.string.live_cancel_admin), "#ffffff", "#f5d83e", false);
                break;
            case WELCOME_TYPE:  //欢迎
                welcomeholder.mLevelView.setVisibility(View.VISIBLE);
                welcomeholder.mLevelView.setLevel(Integer.valueOf(user.getData().getFuser().getUser().getUser_level()));
                tipTemplate(WELCOME_TYPE, welcomeholder, user, context.getString(R.string.live_welcome),
                        context.getResources().getColor(R.color.comm_text_color_main),
                        context.getResources().getColor(R.color.comm_live_in_room), true, false);
                break;
            case Rebot_Join:  //机器人进入
                rebotJoinholder.mLevelView.setVisibility(View.VISIBLE);
                rebotJoinholder.mLevelView.setLevel(Integer.valueOf(user.getData().getUser().getUser_level()));
                tipTemplate(Rebot_Join, rebotJoinholder, user, context.getString(R.string.live_welcome),
                        context.getResources().getColor(R.color.comm_text_color_main),
                        context.getResources().getColor(R.color.comm_live_in_room), true, true);
                break;
            case LEAVE_TYPE:  //用户离开
                leaveHolder.mLevelView.setVisibility(View.VISIBLE);
                leaveHolder.mLevelView.setLevel(Integer.valueOf(user.getData().getUser().getUser_level()));
                tipTemplate(LEAVE_TYPE, leaveHolder, user, context.getString(R.string.live_levae_room),
                        context.getResources().getColor(R.color.comm_text_color_main),
                        context.getResources().getColor(R.color.comm_live_out_room), true, false);
                break;
            case FOLLOW_TYPE:  //关注
...
//                tipTemplate(shareSucHolder, user, context.getString(R.string.live_sha_suc), "#ffffff", "#f5d83e", true);
                break;
            case RICH_LV_TYPE:  //用户升级
                inflateContent(hnRichLvHolder.mMsg, context.getString(R.string.live_sys_info),
                        String.format(user.getMsg()),
                        context.getResources().getColor(R.color.comm_live_live_up),
                        context.getResources().getColor(R.color.comm_live_live_up));
//                HnReceiveSocketBean.DataBean data = user.getData();
//                if (data != null) {
//                    HnReceiveSocketBean.DataBean.UserBean fuser = user.getData().getUser();
//                    if (fuser != null) {
//                        String nick = fuser.getUser_nickname();
//                        String richlvl = fuser.getUser_level();
//                        inflateContent(hnRichLvHolder.mMsg, context.getString(R.string.live_sys_info),
//                                String.format(context.getString(R.string.live_congri_user_lv), nick, richlvl),
//                                context.getResources().getColor(R.color.comm_live_live_up),
//                                context.getResources().getColor(R.color.comm_live_live_up));
//                    }
//                }
