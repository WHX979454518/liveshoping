...
public class HnStartServiceActivity extends BaseActivity {
    ...
    public void onCreateNew(Bundle savedInstanceState) {
        ...
        if(bundle!=null) {
            ...
            if (WEBSCOKET_SERVICE.equals(type)) {
                String mWebsocketUrl = bundle.getString("websocket_url");
                if (!TextUtils.isEmpty(mWebsocketUrl)) {
                    Intent intent = new Intent(this, HnWebSocketService.class);
                    Bundle bun = new Bundle();
                    bun.putString("websocket_url", mWebsocketUrl);
                    intent.putExtras(bun);
                    startService(intent);
                } else {
                    HnLogUtils.e(TAG, "wenscketurl 为空，无法启动websocket服务");
                }
            }
        }
        finish();
