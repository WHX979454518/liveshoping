...
public class HnLiveSwitchDataUitl {
    ...
    public static void joinRoom(final Context context, final String anchorId, final String store_id) {
        ...
        HnHttpUtils.postRequest(HnUrl.LIVE_ROOM_SWITCH, params, HnUrl.LIVE_ROOM_SWITCH, new HnResponseHandler<HnLiveRoomSwitchModel>(HnLiveRoomSwitchModel.class) {
                ...
                if (datas != null && datas.size() > 0) {
                    ...
                    for (int i = 0; i < datas.size(); i++) {
                        ...
                        HnLiveListModel.LiveListBean bean = new HnLiveListModel.LiveListBean("",
                                datas.get(i).getUser_id(), datas.get(i).getAnchor_live_img());
                        list.add(bean);
                    }
                    model.setList(list);
            ...
            public void hnErr(int errCode, String msg) {
                ...
                if (TextUtils.equals("0", store_id) || TextUtils.isEmpty(store_id)) {
                    HnToastUtils.showToastShort(msg);
                    return;
                } else {
                    ShopActFacade.openShopDetails(store_id);
                }
